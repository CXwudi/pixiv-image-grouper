...
def get_year(entry: DirEntry) -> str: 
  ...
  if year_from_pixiv_id == id_not_found_str:
    log.info(f"can not find pixiv id from filename {entry.name}, use last modified year {year_from_lm}")
    return year_from_lm
  else:
...
def try_get_year_from_pixiv_id(entry: DirEntry) -> str:
  ...
  raise InvalidPixivIdError(f"pixiv id {pixiv_id} from filename {entry.name} is not in any range")
